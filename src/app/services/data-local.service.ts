...
export class DataLocalService {
  ...
  agregarPropietario(id: string, nombres: string, apellidos: string, edad: number, ocupacion: string, direccion: string, email: string, celular: number, fecha: Date, administrador: number){
    this.eliminarTodos();
    //this.navCtrl.navigateForward('/tabs/tab2');
    const nuevoPropietario = new PropietarioLocal(id, nombres, apellidos, edad, ocupacion, direccion, email, celular, fecha, administrador);
